...
import { dealerBottomPerformers } from "@/lib/mock-data"
...
const CustomLegend = () => (
  <div className="flex items-center justify-center gap-8 text-xs">
    <div className="flex items-center gap-2">
      <div className="w-2.5 h-2.5 rounded-full bg-[#E67E93]" />
...
export const DealerBottomPerformers = () => {
  const data = [...dealerBottomPerformers]
    .sort((a, b) => a.totalGLA - b.totalGLA)
    .map(dealer => ({
      name: dealer.name,
    ...
    return (
      <div className="bg-white/90 backdrop-blur-sm rounded-lg border border-[#4B2D84]/20 shadow-sm p-3">
        ...
        {payload.map((entry: any, index: number) => {
          ...
          const metrics = dealerBottomPerformers.find(d => d.name === label);
  ...
  return (
    <Card className="h-full overflow-hidden bg-white">
      <div className="px-6 pt-4 pb-6 h-full flex flex-col">
        <Title className="text-[#4B2D84] font-medium text-sm mb-4">Bottom Performing Dealers</Title>
              ...
              >
                ...
                >
                  <LabelList
                    ...
                    style={{ fill: "#E67E93", fontSize: "11px" }}
                  />
                </Bar>
                <Bar
                  dataKey="Deals"
